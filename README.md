...
1. **材料来源**：事实从哪里来，决定归因和风险规则。
2. **本次任务范围**：用户这次具体要什么，决定执行哪些步骤以及在哪里停止。
...
单步骤默认直接在对话中交付，不创建分析目录或 `evidence-manifest.json`。缺少输入时只索取当前步骤所需的最小材料；如果确实需要扩大范围，会先说明原因并等待确认。多个明确任务可以组合，例如“改两条简历并准备话术”，但不会自动插入未请求的分析、增强、拷打或学习步骤。
...
$repo-to-resume 只改这一条简历：使用 Redis 优化接口，性能提升 80%。
...
当前回归套件包含 6 个代表性场景、56 条评估断言和 15 条负向契约测试，覆盖弱实习、项目迁移、现有简历、范围路由、版本化产物、中文输出一致性和未授权源码修改等关键路径。`test-prompts.json` 包含 13 条真实前向测试提示词，其中 3 条专门验证“只分析业务链路、只改一条简历、只做模拟面试”不会扩张成完整流程。

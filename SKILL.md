...
8. 任何生成或改写简历要点的任务都先确认项目来源；来源未确认时只问来源并停止，不输出示例、候选或改写稿。
...
5. 单个 `*_ONLY` 默认使用 `DIRECT` 执行：在对话中交付，不创建分析目录或 manifest。用户明确要求落盘，或多个阶段需要共享事实状态时，切换为 `ARTIFACT` 执行。
...
进入本阶段前必须已有明确 `project_origin`。缺失时回到 `CHECKPOINT · PROJECT ORIGIN`，只问来源并停止。`DIRECT + RESUME_ONLY` 在来源确认后可直接处理用户提供的要点或事实，不要求 manifest、候选池或全仓扫描；只对角色、数字口径、因果关系等会改变当前表述真实性的缺口做一次一个的最小追问。没有证据的数字提供“待补证据占位版”和“删除数字稳妥版”，不得把单条改写扩张为六层拷打。

...
如果用户明确授权写文件，则先展示拟修改位置、事实改写范围、输出文件名和验证方式。授权只覆盖用户点名的简历文件，不覆盖项目源码或其他材料；输出使用 `-vN` 或明确的新文件名，禁止覆盖原文件。写入前扫描同名版本并取最大版本号加一，先写同目录临时文件，完成格式解析与可用验证后再原子重命名为最终版本；原文件全程保持不变。纯加粗/间距等格式修订可直接使用已确认文字；职责、角色或项目事实改写仍须通过 `PROJECT ORIGIN` 门禁。
...
| `FILE_REVISION` 相邻目录不可写 | 展示目标路径并请求一个可写输出目录 | 只保留审查建议，不改原文件或改写权限 |
| `-vN` 在写入前被占用 | 重新扫描最大版本号并选择下一个 | 改用时间戳后缀；仍冲突则停止写入 |
| 修订版解析、编译或渲染验证失败 | 保留原文件并报告失败位置，不发布临时文件 | 退化为定位明确的修改建议 |

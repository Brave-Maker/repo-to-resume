---
name: repo-to-resume
description: 将本地路径、GitHub 仓库、开源/他人项目、真实或薄弱实习、自研项目和现有简历转化为可面试的项目经历。既可只执行代码分析、贡献映射、经历增强、简历改写、逐条拷打、学习路径、面试话术、模拟面试或质量检查中的一个步骤，也可在用户明确要求时执行完整流程。用户提到“分析项目”“看懂代码”“写简历”“只改一条”“没有实习怎么包装”“开源项目包装”“项目面试”“STAR”“拷打简历”“模拟面试”“项目答辩”或提供项目路径/Git URL 时使用。
---
...
5. 完整流程中的每个核心简历要点都要通过事实、角色、实现、选型、验证和边界六层追问；单条改写只核验影响当前表述真实性的最小必要事实。
6. 需要落盘的阶段产出文件并保留版本，不得覆盖已有文件；用户只要求对话结果时不为流程完整性强制写文件。
...
1. `source_mode`：材料来自哪里，决定事实与归因规则。
2. `task_scope`：用户这次要什么，决定执行哪些阶段以及在哪里停止。
...
来源不明确但不影响当前单步骤时不要追问。例如，用户只要求改写已提供的一句话，不必先判断它来自实习还是自研项目。

`ARTIFACT` 执行沿用 manifest 现有的 `mode` 字段存储 `source_mode`，不要新增 schema 未声明的字段。只有分析、不涉及经历来源时可用兼容值 `UNDERSTAND_ONLY`。`task_scope` 属于本次执行计划，记录在对话与产物摘要中，不写入当前 manifest schema。
...
| `LEARNING_ONLY` | 只生成学习路线、只补知识 | Phase 6 |
...
| `RESUME_ONLY` | 用户提供的事实、要点或已有简历 | 只追问影响当前表述真实性的事实 |
| `GRILL_ONLY` | 待拷打的要点 | 直接逐条追问，一次一个问题 |
| `LEARNING_ONLY` | 目标主题或风险清单 | 只索取学习目标与时间约束 |
...
2. 列出本次会执行的阶段、明确跳过的阶段和停止点；显式“只/不要/不需要”是硬边界。
3. 确定输出语言：用户明确指定 > 用户当前使用的语言 > 默认中文；仅在范围包含代码分析时确认扫描范围与跳过目录。
4. 单个 `*_ONLY` 默认使用 `DIRECT` 执行：在对话中交付，不创建分析目录或 manifest。用户明确要求落盘，或多个阶段需要共享事实状态时，切换为 `ARTIFACT` 执行。
5. `ARTIFACT` 执行才在被分析项目根目录创建 `{项目名}-analysis/`，并按 `references/evidence-manifest.schema.json` 初始化 `evidence-manifest.json`。若检测到 `schema_version=1.0`，先运行 `python scripts/migrate_manifest.py <manifest> --output <临时文件>`，校验迁移结果并让用户确认差异后再原子替换；其他旧版本停止并报告不支持。`1.1` 版本先运行 `python scripts/validate_manifest.py <manifest>`，通过后增量更新。

🔴 CHECKPOINT · MODE & SCOPE

展示 `source_mode`、`task_scope`、执行阶段、停止点、预计产物和风险路径后暂停。用户已明确给出单步骤边界且不涉及写代码或范围扩张时，可直接执行；`FULL_PIPELINE` 或组合多个范围时等待确认。用户已授权自动完成时写入自动确认记录并继续。
...
`DIRECT + RESUME_ONLY` 可直接处理用户提供的要点或事实，不要求 manifest、候选池或全仓扫描；只对角色、数字口径、因果关系等会改变当前表述真实性的缺口做一次一个的最小追问。没有证据的数字提供“待补证据占位版”和“删除数字稳妥版”，不得把单条改写扩张为六层拷打。
...
3. 按来源和角色选择动词。
4. 将高风险声明保留在草稿时，写入内部 claim ID 和风险，不在用户可见正文堆内部标签。
5. 数字沿用 manifest 的来源与口径；无数字时使用范围、闭环、故障或验证结果。提交文件数、增删行数、commit 大小和文档数量只属于审计元数据，不能充当结果或含金量证明。
6. 生成当前版本简历，更新 `current_artifacts.resume`；最多 10 条简历要点，每条保持紧凑。
7. 中文成品执行术语本地化：使用“简历要点、意图、大语言模型、智能体、数据结构约束、检查点、行动与结果”等自然中文，不把 `Bullet`、`Intent`、`LLM`、`Agent`、`Schema`、`checkpoint`、`Action & Result` 当作普通中文词直接混排。
8. 只为准确性保留必要英文：产品/框架/协议/行业通用缩写和真实代码标识符。代码标识符使用反引号并在首次出现时补中文职责，如 `decompose`（步骤拆分）；确需后续使用缩写时首次写“大语言模型（LLM）”，否则只写中文。
9. 发布前逐句检查标题和正文；发现可翻译的通用英文术语或用 Git 规模冒充成果时自动改写一次，仍不合格则标记 `REVIEW_REQUIRED`，不得称为最终版。

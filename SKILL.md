...
2. 直接沿用贡献映射阶段的 12 分 `candidate_value_score` 从高到低选择，不在简历阶段另建评分公式；角色、来源、掌握度和风险只决定是否准入。任何未入选的合格候选分数更高时，必须替换较弱要点；因岗位不相关或内容多样性不替换时，在内部索引写明理由。
3. 按来源和角色选择动词。
...
| 入选要点价值低于未入选合格候选 | 用高价值候选替换并重新检查角色 | 不能替换时记录角色风险或多样性理由，状态最高为 `REVIEW_REQUIRED` |
